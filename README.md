...
**Trigger watch:** [`docs/TRIGGER_WATCH.md`](./docs/TRIGGER_WATCH.md) logs public LogicFolding developments and scores each against the memo's three reopen triggers. As of 2026-07-03 (Tau Scaling Law V2 paper), none has fired and the no-action decision stands.
...
When a teardown does arrive (**Trigger A**, expected no earlier than the fall 2026 Mate 90 / Kirin 9050 Pro shipment), inter-wafer process variation and stacked-die yield join sustained-thermal behavior and the fraction of logic actually folded on the measurement checklist. The 2026-05-30 trigger-watch entry logs this as a teardown variable rather than a model parameter; the 2026-07-03 entry adds Huawei's self-reported Kirin 2026 power/area data as a claimed fixture (not verified truth).

## Layout

- `python/` — Eq. 2 engine + 2D-baseline ingestion. `core_solver.py` holds `VerticalPathEvaluator` (the path-level break-even inequality, memo §7 Eq. 2); `baseline.py` parses the `logic-folding-baseline/v0` contract (OpenROAD / OpenSTA output) into that engine, merges real routed wire length from odb, and joins it to the 3D tax. `scripts/kirin_2026_whatif.py` runs a "what-if" break-even analysis against Huawei's claimed Kirin 2026 data (clearly marked as unverified). Pytest suite included.
